...
class Architecture:
    ...
    def __init__(self, input_tapes, output_tapes, blocks, structures=None, layer=0, horizontal=False):
        if not horizontal:
            ...
            if not structures and structures != []:
                ...
                arch_structure = Architect.get_parent_last_derivate_blocks(self, structure)
    ...
    def set_structures_error(self, error_after, penalties):
        ...
        if self.last_structure_changed != -1 and self.structures[self.last_structure_changed].is_better(error_after, penalties, lastone):
            self.structures[self.last_structure_changed].update_error(error_after, penalties, lastone)
            ...
            if not self.horizontal or self.horizontal_index == -1:
                ...
                self.optimize_current_horizontal_architecture()
                self.structures[self.last_structure_changed].reset(confirm_code=True, more_over=self.get_code(self.horizontal[self.horizontal_index].generate()))
    ...
    def optimize_current_horizontal_architecture(self):
        architecture = self.horizontal[self.horizontal_index].copy()
        architecture
        architecture.input_tapes, architecture.output_tapes = self.get_all_variable_at_state()

    def get_all_variable_at_state(self):
    ...
    def copy(self):
        return Architecture(self.input_tapes,self.output_tapes,self.ground_blocks,structures=self.structures, layer=self.layer,horizontal=self.horizontal)
...
#
    ...
    def is_better(self, new_error, penalties, constant):
        if constant:
            ...
            if max(penalties) >= BIG_NUMBER:
                ...
                if self.level == 0:
                    print("sqrt: " + str(np.sqrt(np.std(new_error))) + " ,sums = " + str(sum(new_error) + sum(penalties)) + ", product:" + str(np.sqrt(np.std(new_error)) * (sum(new_error) + sum(penalties))))
    ...
    def reset(self, confirm_code=False, more_over=""):
        if confirm_code:
