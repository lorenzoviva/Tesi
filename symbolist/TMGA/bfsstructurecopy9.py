...
class Architecture:
    ...
    def __init__(self, input_tapes, output_tapes, blocks, structures=None, child=None):
        ...
        if isinstance(blocks, (list,)):
            self.ground_blocks = blocks
        else:
            self.ground_blocks = Structure.from_str(str(blocks),0).building_blocks
    ...
    def step(self):
        ...
        error_after, difference_of_error, total_error_after, penalties = self.get_multiple_input_error(self.current_error, code)
    ...
    def get_structures(self):
        ...
        while x > 0:
            if len(structures) == 0 or not isinstance(structures[0], (Structure,)):
                structures = [Structure(self.ground_blocks, level=current_level)] + structures
            if x % 2:
...
class Structure:
    ...
    def append_blocks(self):
        if self.parent:
            self.parent.input_tapes.append(str(self))
            self.parent.step()
        else:
...
#
    ...
    def expand(matched_text):
        ...
        index = indexes.pop(0)
        while index and matched_text[index] != "(":
            if matched_text[index] not in effect:
                matched_text = matched_text[:index+1] + effect + matched_text[index+1:]
            else:
                pass
                # matched_text = matched_text[:index] + effect + matched_text[index+1:]
            index = indexes.pop(0)
        matched_text = matched_text[:index] + matched_text[index+1:]
        return matched_text
