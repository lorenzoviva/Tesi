...
class Architecture:
    ...
    def next(self):
        ...
        if self.copy_enabled:
            multiplier = 4
        if len(self.structures[index].building_blocks) > self.longhest_operation* multiplier or self.structures[index].confirmed_code.count("<'") + self.structures[index].confirmed_code.count(">'") > self.longhest_operation* multiplier:  #
            if index == queue[-1]:
                # if self.copy_enabled:
                self.next_structures()
...
class Structure:
    ...
    def reset_blocks(self):
        ...
        self.building_blocks = [inc_dec_block]
        self.append_blocks()
    ...
    def append_blocks(self):
        ...
        if self.level > 0:
            self.building_blocks = [move_block] + self.building_blocks + [move_block]
            # self.building_blocks = self.building_blocks + [move_block_and_blocks, inc_dec_block]
        else:
            self.building_blocks =  self.building_blocks + [move_block]
    #
    # def reset_blocks(self):
