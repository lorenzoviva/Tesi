...
class Architecture:
    ...
    def set_structures_error(self, error_after, penalties):
        ...
        structure = self.structures[self.last_structure_changed]
        if self.last_structure_changed != -1:
            structure.update_if_better(error_after, penalties, lastone)
...
class Structure:
    ...
    def update_if_better(self, new_error, penalties, constant):
        if self.is_better(new_error, penalties, constant):
            print("\tResetting: " + str(self) + " and confirming code: " + self.generate())
            self.update_error(new_error, penalties, constant)
            print("\tResetted: " + str(self) + " new generated code: " + self.generate())

    def is_better(self, new_error, penalties, constant):
        if constant:
            print("New error1: " + str(sum(new_error) + sum(penalties)))
            ...
            if max(penalties) >= BIG_NUMBER:
                ...
                if self.level == 0:
                    print("New error1: " + str(np.sqrt(np.std(np.array(new_error) - np.array(penalties))) * (sum(new_error) + sum(penalties))))
                    return np.sqrt(np.std(np.array(new_error))) * (sum(new_error) + sum(penalties)) < self.current_error or self.current_error == -1
                else:
                    print("New error2: " + str(np.sqrt(np.std(np.array(new_error) - np.array(penalties))) * (1 + np.mean(penalties))))
                    return np.sqrt(np.std(np.array(new_error) - np.array(penalties))) * (1 + np.mean(penalties)) < self.current_error or self.current_error == -1

    def update_error(self, new_error, penalties, constant):
        self.reset(confirm_code=True)
        if constant:
            ...
            if max(penalties) >= BIG_NUMBER or max(new_error) >= BIG_NUMBER:
                ...
                if self.level == 0:
                    self.current_error = np.sqrt(np.std(np.array(new_error))) * (sum(new_error) + sum(penalties))
                else:
                    self.current_error = np.sqrt(np.std(np.array(new_error) - np.array(penalties))) * (1 + np.mean(penalties))
