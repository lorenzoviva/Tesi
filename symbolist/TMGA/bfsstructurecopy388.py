...
class Architecture:
    ...
    def next(self):
        ...
        if len(self.structures[index].building_blocks) > self.longhest_operation*2+1 or self.structures[index].confirmed_code.count("<'") + self.structures[index].confirmed_code.count(">'") > self.longhest_operation*2+1:  #
            if index == queue[-1]:
                self.next_structures()
                self.last_structure_changed = -1
                self.optimization_queue_index = 0
