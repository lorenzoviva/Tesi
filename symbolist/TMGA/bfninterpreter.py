...
class Interpreter:
    ...
    def step(self):
        ...
        if not jumping:
            ...
            if instruction == "^" and (chr(self.tape.get()) == self.code[self.code_pointer-1] or chr(self.tape.get()) == "^"):  # remove
            ...
            if instruction == "/" and (chr(self.tape.get()) == self.code[self.code_pointer-1] or chr(self.tape.get()) == "/"):
                if chr(self.tape.get()) == "/":
                    self.code_pointer -= 1
                    self.tape.remove()
                    self.tape.left()
