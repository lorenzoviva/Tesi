...
class Architecture:
    ...
    def run(self):
        ...
        self.queue = self.get_optimization_queue()
        ...
        while total_error_after > 1:
            ...
            print("################################################")
            # print("structure:" + self.get_structure_string())
            # print()
    ...
    def set_structures_error(self, total_error_after):
        for i, structure in enumerate(self.structures):
            if isinstance(structure, (Structure,)):
                ...
                #     self.queue.remove(self.last_structure_changed)
                #     self.queue = [self.last_structure_changed] + self.queue
                if structure.is_better(total_error_after):
                    structure.current_error = total_error_after
                    self.reset_structure(i)
                    self.queue.remove(i)
                    self.queue = [i] + self.queue
...
class Structure:
    ...
    def reset(self, confirm_code=False):
        if confirm_code:
            self.confirmed_code = self.generate()
        else:
            self.confirmed_code = ""
    ...
    def __str__(self):
        ...
        return str([str(building_block) for building_block in self.building_blocks])
