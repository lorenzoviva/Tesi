...
class Building:
    ...
    def __init__(self, architectures=None, blocks=None):
    ...
    def join(self, input_tapes, output_tapes, architectures=None):
        if architectures is None:
            architectures = self.architectures
        self.iterations += 1
        found = False
        if not architectures:
            architecture = Architecture(input_tapes, output_tapes, self.blocks, max_layers=self.layers)
        else:
            architecture = Architecture(input_tapes, output_tapes, None, horizontal=self.architectures)
        for i in range(self.COMPLEXITY_STEP):
            if architecture.horizontal:
                architecture.reset_architectures()
            architecture.run_to_first_reset()
            if sum(architecture.current_error) < 1:
                if architecture.horizontal:
                    architecture = Architect.get_vertical_from_horizontal(architecture)
                self.architectures.append(architecture)
                found = True
                break
        if not found and architecture.horizontal:
            return self.join(input_tapes, output_tapes, [])
        if not found:
            if self.COMPLEXITY_STEP not in self.unappreciated.keys():
                self.unappreciated[self.COMPLEXITY_STEP] = []
            self.unappreciated[self.COMPLEXITY_STEP].append([input_tapes, output_tapes])
        return found
...
class Architect:
    ...
    @staticmethod
    def get_vertical_from_horizontal(architecture):
        architecture_copy = architecture.copy()
        # base_structure = Architect.insert_parent_structure(architecture.structures+".")
        structures = []
        level = 0
        for i, structure in enumerate(architecture_copy.structures):
            if isinstance(structure, (Structure,)):
                if structure.generate():
                    new_structure = Architect.insert_parent_structure(structure.generate())
                    new_structure.next()
                    structures += [new_structure]
                elif architecture_copy.horizontal and architecture_copy.horizontal_index != -1 and architecture_copy.last_structure_changed == i:
                    # arch1 = architecture_copy.horizontal[architecture_copy.horizontal_index]
                    # new_structure = Structure.from_str("." +arch1.generate() + ".,;", level)
                    # new_structure.next()
                    structures += architecture_copy.horizontal[architecture_copy.horizontal_index].structures
                else:
                    structures += [Structure([], level)]
            else:
                if structure == "[":
                    level += 1
                else:
        ...
        architecture_copy.structures = structures
        architecture_copy.horizontal = []
        architecture_copy.horizontal_index = -1
        architecture_copy.run_to_first_reset()
        return architecture_copy

    @staticmethod
    def get_parent_all_derivative_blocks(architecture, structure):
        if architecture.layer < architecture.MAX_LAYERS:
            return Architecture(architecture.input_tapes, architecture.output_tapes, Architect.derivative_parent_blocks(structure), layer=architecture.layer + 1)
        else:
            return None

    @staticmethod
    def get_parent_last_derivative_blocks(architecture, structure):
    ...
    @staticmethod
    def get_parent_last_derivative_structure(architecture, structure):
        if architecture.layer == architecture.MAX_LAYERS - 1:
            return Architecture(architecture.input_tapes, architecture.output_tapes, Architect.derivative_parent_structure(structure), layer=architecture.layer + 1)
        elif architecture.layer < architecture.MAX_LAYERS - 1:
    ...
    @staticmethod
    def get_parent_all_derivative_structure(architecture, structure):
        if architecture.layer < architecture.MAX_LAYERS:
            return Architecture(architecture.input_tapes, architecture.output_tapes, Architect.derivative_parent_structure(structure), layer=architecture.layer + 1)
        else:
            return None
    ...
    @staticmethod
    def derivative_parent_blocks(structure):
        result = ""
        for block in structure.building_blocks:
            result += ".("+str(block) + ",)'."
        for block in structure.building_blocks:
    ...
    @staticmethod
    def derivative_parent_structure(structure):
        result = ""
        result += ".("+str(structure) + ")'."
        result += ".[<]>("+str(structure) + ")^."
        result += ".("+str(structure) + ")*."
...
class Architecture:
    ...
    def __init__(self, input_tapes, output_tapes, blocks, structures=None, layer=0, horizontal=None, max_layers=2):
        self.temp_structure = None
        if not horizontal:
            ...
            if not structures and structures != []:
                ...
                arch_structure = Architect.get_parent_last_derivative_structure(self, structure)
    ...
    def step(self):
        ...
        if not self.horizontal:
            self.code = Architecture.get_code(self.code_writing_code)
        else:
    ...
    def generate_parent(self):
        ...
        self.iterations += 1
        self.code_writing_code = self.generate()
        self.code = str(Architecture.get_parent_output(self.code_writing_code, self.input_tapes[-1]))
        if DEBUG:
    ...
    def set_structures_error(self, error_after, penalties):
        ...
        if self.last_structure_changed != -1 and structure.is_better(error_after, penalties, lastone):
            print("EVOLUTION: the error is getting better after the tranformation on " + str(self))
            is_better = True
            ...
            if not self.horizontal or self.horizontal_index == -1:
                ...
                if DEBUG:
                    print("\tResetted: " + str(structure) + " new generated code: " + structure.generate())
            else:
                architecture = self.optimize_current_horizontal_architecture(error_after, penalties)
                if DEBUG:
                    print("\tResetting: " + str(structure) + ", confirming code: " + structure.generate() + " and setting as parent of this structure, the architecture that generate the following code:" + architecture.generate_parent())
                structure.reset(confirm_code=True, architecture=architecture)
                if DEBUG:
                    print("\tResetted: " + str(structure) + " new generated code: " + structure.generate() + " and set as parent of this structure, the architecture that generate the following code:" + architecture.generate_parent())
        return is_better


    def print_situation(self, error_after, difference_of_error, total_error_after,  code, message=""):
        if self.horizontal:
            print("HORIZONTAL " + str(self.horizontal) + " HI:" + str(self.horizontal_index))
        ...
        temp_str = str(self.temp_structure)
        if self.temp_structure:
            temp_str += " " + self.temp_structure.confirmed_code
        print("\tTemporary structure:\t " + temp_str)
        print("\n\nSTRUCTURE:\n" + str(self) + "\n")
        print("################################################\n\n\t\t\t\t"+message+"\n\n################################################")
        ...
        for i, structure in enumerate(self.structures):
            if isinstance(structure, (Structure,)):
                ...
                if self.horizontal and self.horizontal_index != -1 and self.last_structure_changed == i:
                    ...
                    code += Architecture.get_code(arch1.generate())
            else:
                code += structure
        return code
    ...
    def next(self):
        if self.horizontal and self.horizontal_index == -1:
            ...
            spatial_complexity_reached = (not self.horizontal and len(regex.findall(self.structures[index].generate())) > self.longhest_operation + 1) or (self.horizontal and len(self.structures[index].building_blocks) > self.longhest_operation + 1)
            is_last_strucutre = index == queue[-1]
            havent_tried_without_confirmed_code = self.horizontal and self.horizontal_index != -1 and not self.temp_structure
            havent_finished_horizontal = self.horizontal and self.horizontal_index < len(self.horizontal) - 1
            temporary_structure_error_is_better = self.temp_structure and (self.structures[index].current_error == -1 or self.structures[index].current_error > self.temp_structure.current_error)
            temporary_structure_error_is_worst = self.temp_structure and not temporary_structure_error_is_better

            if spatial_complexity_reached:

                if havent_finished_horizontal:
                    if havent_tried_without_confirmed_code:
                        self.temp_structure = self.structures[index].copy()
                        architecture = self.optimize_current_horizontal_architecture([-1], [0])
                        self.structures[index].parent = architecture
                        self.structures[index].reset()
                        self.structures[index].confirmed_code = ""
                        self.structures[index].current_error = -1
                    elif temporary_structure_error_is_better:
                        wrost = self.structures[index].copy()
                        self.structures[index] = self.temp_structure.copy()
                        self.temp_structure = wrost
                    # elif temporary_structure_error_is_worst:
                    #     self.temp_structure = None
                    else:
                        self.horizontal_index += 1
                        self.current_error = list(np.ones(len(self.input_tapes))*-1)
                ...
                reset = self.structures[index].next()
                if reset:
                    self.reset_architectures()
                self.last_structure_changed = index
    ...
    @staticmethod
    def get_code(code_writing_code):
    ...
    @staticmethod
    def get_parent_output(code, input_tape):
...
class Structure:
    ...
    def split_blocks(text):
        ...
        while text:
            ...
            while found_comma != -1 and len(text) > found_comma+1 and text[found_comma+1] in ESCAPE_LIST:
    ...
    def copy(self):
        ...
        structure.horizontal = self.horizontal
        return structure
...
class BuildingBlock:
    ...
    def next(self):
        ...
        next_dot = tape_text.find(".")
        while next_dot != -1 and len(tape_text) > next_dot+1 and tape_text[next_dot+1] in ESCAPE_LIST:
            next_dot = tape_text.find(".", next_dot+1)
        return next_dot == -1
