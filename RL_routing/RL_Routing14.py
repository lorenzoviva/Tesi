...
def optimize_model():
    ...
    prev_state_vector = torch.zeros((BATCH_SIZE, 1 ,4,4), device=device)
    ...
    action_batch = torch.cat(batch.action)
    state_action_values.append(policy_out[0].gather(1, action_batch))
    ...
    expected_state_action_values.append((next_state_values * GAMMA) + reward_batch)
    # run simulation
    n_step = 0
    while n_step < simulation_steps:
        target_out = target_net(generated_post_state)
        policy_out = policy_net(generated_pre_state)
        next_state_vector[non_final_mask, : ,:, :] = target_out[1].gather(1, target_out[0].max(1)[1].repeat(1,16).reshape((target_out[0].shape[0],1,4,4)))
        non_final_action_batch = torch.zeros(BATCH_SIZE, dtype=torch.long, device=device)
        non_final_action_batch[non_final_mask] = policy_out[0][non_final_mask].max(1)[1]
        prev_state_vector[non_final_mask, : ,:, :] = policy_out[1][non_final_mask].gather(1, non_final_action_batch[non_final_mask].repeat(1, 16).reshape((policy_out[0][non_final_mask].shape[0], 1, 4, 4)))
        post_transaction_input_feature_map = next_state_vector[non_final_mask].view(target_out[1].shape[0], np.prod(target_out[1].shape[2:]), 1,1)
        pre_transaction_input_feature_map = prev_state_vector.view(policy_out[1].shape[0], np.prod(policy_out[1].shape[2:]), 1,1)
        generated_post_state = generator(post_transaction_input_feature_map)
        generated_pre_state = generator(pre_transaction_input_feature_map)
        next_state_values[non_final_mask] = target_out[0].max(1)[0].detach()
        state_action_values.append(policy_out[0].gather(1, non_final_action_batch.unsqueeze(1)))
        #next_state_values = (next_state_values / (next_state_values.max(0)[0] - next_state_values.min(0)[0]))
        expected_state_action_values.append((next_state_values * np.power(GAMMA, n_step)))
        n_step += 1

    # losses = -expected_state_action_values.unsqueeze(1) * F.log_softmax(state_action_values, dim=-1)  # CE
    # loss = losses.sum(dim=-1).mean()  # sum of classes, mean of batch
    loss = F.smooth_l1_loss(torch.cat(state_action_values), torch.cat(expected_state_action_values).unsqueeze(1))
