...
class Policy(nn.Module):
    ...
    def __init__(self):
        ...
        # actor's layer
        self.action_head = nn.Linear(128, 4) # 1
        ...
        # )
        self.state_action_routing = nn.Sequential(
            Routing(d_cov=1, d_inp=2, d_out=2, n_out=1),
        )
        # self.state_action_routing = nn.Sequential(
    ...
    def forward(self, x):
        ...
        action_caps = self.action_head(x).view(x.shape[0],-1,1,2)
    ...
    if max_image_height != -1:
        ...
        # summary = torch.cat(images,-1)
            ...
            if len(image.shape) > 2:
                plt.imshow(image.permute(1,2,0).detach().numpy(), interpolation='none', animated=True)#, )
            else:
                plt.imshow(image.permute(1,0).detach().numpy(), interpolation='none', animated=True, cmap='cividis')
                plt.colorbar()
    # im = None
    #
    # plt.colorbar()
