...
class RoutingModel(nn.Module):
    ...
    def __init__(self, n_objs, n_parts, d_chns):
        ...
        self.head = nn.Linear(d_chns, n_objs)
...
policy_net = RoutingModel(2, 1, 32).cuda(device) # RoutingLayer(screen_height, screen_width, n_actions).to(device)
target_net = RoutingModel(2, 1, 32).cuda(device) # RoutingLayer(screen_height, screen_width, n_actions).to(device)
