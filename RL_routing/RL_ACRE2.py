...
class Policy(nn.Module):
    ...
    def forward(self, x):
        ...
        return action_prob.squeeze(), state_prob, action_caps, state_caps
...
def select_action(state):
    ...
    action_probs, state_prob, action_caps, state_caps = model(state.unsqueeze(0))

    # create a categorical distribution over the list of probabilities of actions
    ms = Categorical(state_prob)

    state_value = ms.sample()

    ma = Categorical(action_probs[state_value,:])
    ...
    model.saved_actions.append(SavedAction(ma.log_prob(action), ms.log_prob(state_value),  ms.entropy()))
...
def finish_episode():
    ...
    for (log_prob, value, entropy), R in zip(saved_actions, returns):
        ...
        if torch.isnan(entropy):
            ...
            value_losses.append(F.smooth_l1_loss(value, torch.tensor([R]))*entropy)

        # value_losses.append(-value * torch.tensor([R]))
        act.append(value)
