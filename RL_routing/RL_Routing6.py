...
class RoutingModel(nn.Module):
    ...
    def forward(self, images):
        ...
        # a_max = a.max(1)[1]
        # mu_a_max = mu[a_max].view((mu.shape[0],-1))
        # a = self.head(mu_a_max)
        return a, mu, sig2
...
policy_net = RoutingModel(2, 15, 4, 2).cuda(device) # RoutingLayer(screen_height, screen_width, n_actions).to(device)
target_net = RoutingModel(2, 15, 4, 2).cuda(device) # RoutingLayer(screen_height, screen_width, n_actions).to(device)
# decoder = Decoder(2, 16, 40, 90, 1, True)
generator = Generator(1, 16, 8, 3).to(device)
...
def plot_durations(generated_pre_transition, generated_post_transition, batch):
    ...
    if generated_post_transition is not None:
        randi = random.randint(0, generated_post_transition.shape[0] - 1)
        fig = plt.figure(3)
        plt.clf()
        ...
        target_out_deformed = F.interpolate(F.interpolate(target_out[1].gather(1, post_action_batch)[randi].repeat(3,1,1).cpu(), scale_factor=16).permute(0,2,1), scale_factor=16).permute(0,2,1)
        ...
        image_generated_post_transition = generated_post_transition[randi].cpu().squeeze(0).permute(1, 2, 0).detach().numpy()
        image_generated_pre_trainsition = generated_pre_transition[randi].cpu().squeeze(0).permute(1, 2, 0).detach().numpy()
        image_original_post_transition = original_post_images[randi].cpu().squeeze(0).permute(1, 2, 0).detach().numpy()
        ...
        plt.title(str(randi) + (" -->" if target_out[0].max(1)[1][randi] else " <--"))#+"\n from vector:\n" + str(target_out_deformed))
        plt.imshow(image_out_latent_vector, interpolation='none')
        fig.add_subplot(2, 3, 6)
        plt.title(str(randi) + (" -->" if batch.action[randi] else " <--"))#+"\n from vector:\n" + str(target_out_deformed))
...
def optimize_generator():
    ...
    target_out = target_net(non_final_next_states)
    next_state_values[non_final_mask, : ,:, :] = target_out[1].gather(1, target_out[0].max(1)[1].detach().repeat(1,16).reshape((target_out[0].shape[0],1,4,4)))
