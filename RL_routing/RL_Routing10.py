...
class RoutingModel(nn.Module):
    ...
    def __init__(self, n_objs, n_parts, d_chns, n_out):
        ...
        self.convolve = nn.Sequential(
            *[m for (inp_ch, out_ch, stride) in zip([6] + [d_chns] * 5,  [d_chns] * 6,  [1, 2] * 3)
    ...
    def add_coord_grid(self, x):
        ...
        coord_grid = torch.stack((
            torch.linspace(-1.0, 1.0, steps=h, device=x.device)[None, :, None].expand(t, -1, w),
            torch.linspace(-1.0, 1.0, steps=w, device=x.device)[None, None, :].expand(t, h, -1),
            torch.linspace(-1.0, 1.0, steps=t, device=x.device)[:, None, None].expand(-1, h,w),
        )).permute(1,0,2,3).reshape((t,) + (3, w,h)) # (.expand([*x.shape[:-3], -1, -1, -1])
        return torch.cat((x, coord_grid), dim=-3)

    def forward(self, images):
        #images = torch.stack(images)
        x = self.add_coord_grid(images).cuda(device)
        x = x.reshape((1,) + x.shape[1:-1] + (-1,))
        x = self.convolve(x)                                   # [bs, d_chns, m', n' *t]

        a = self.compute_a(x)                                  # [bs, n_parts, m', n' *t]
...
def plot_durations(generated_pre_transition, generated_post_transition, batch):
    ...
    if generated_post_transition is not None:
        ...
        plt.title(str(randx) + (" -->" if target_out[0].max(1)[1][randx] else " <--"))#+"\n from vector:\n" + str(target_out_deformed))
...
def optimize_model():
    ...
    non_final_next_states = torch.cat([s for s in batch.next_state
                                       if s is not None])
    state_batch = torch.cat([torch.cat(l) for l in batch.state])
    state_batch = [torch.stack(l) for l in batch.state]
    action_batch = torch.cat(batch.action)
    next_state_values = torch.zeros((BATCH_SIZE, 1, 4, 4), device=device)
...
for i_episode in range(num_episodes):
    ...
    # state = (current_screen*0.3 + last_screen*0.7)
    state = [get_screen()]
    for t in count():
        start = time.time()
        # Select and perform an action
        action = select_action(torch.cat([s for s in state]))
        ...
        # current_screen = get_screen()
        current_screen = (get_screen()*0.3 + last_screen*0.7)
        if not done:
        ...
        memory.push(state, action, next_state, reward, time)
