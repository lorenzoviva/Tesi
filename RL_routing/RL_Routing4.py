...
policy_net = RoutingModel(1, 1, 4, 2).cuda(device) # RoutingLayer(screen_height, screen_width, n_actions).to(device)
target_net = RoutingModel(1, 1, 4, 2).cuda(device) # RoutingLayer(screen_height, screen_width, n_actions).to(device)
...
def plot_durations(generated_image, original_images):
    plt.figure(2)
    plt.clf()
    ...
    if generated_image is not None:
        randi = random.randint(0,BATCH_SIZE-1)
        plt.figure(3)
        plt.clf()
        ...
        vstack2 = np.vstack([1 - generated_image[randi].cpu().squeeze(0).permute(1, 2, 0).detach().numpy(), original_images[randi].cpu().squeeze(0).permute(1, 2, 0).detach().numpy()])
        hstack = np.hstack([vstack1,vstack2])
        plt.imshow(hstack,
                   interpolation='none')
        plt.title('Example reconstructed screen: ' + str(randi))
        plt.show()
...
def optimize_generator():
    ...
    input_feature_map = policy_out[1].view(policy_out[1].shape[0],np.prod(policy_out[1].shape[1:]), 1,1)
    ...
    #     param.grad.data.clamp_(-1, 1)
    optimizer_g.step()
    return generated_state, state_batch
######################################################################
#
...
for i_episode in range(num_episodes):
    ...
    for t in count():
        ...
        generated_image, original_images = (None, None)
        if verbose:
            generated_image, original_images = verbose
        end = time.time()
        print("time 3: {:1.10f}".format(end - start))
        start = time.time()
        if done:
            episode_durations.append(t + 1)
            plot_durations(generated_image, original_images)
