...
class Policy(nn.Module):
    ...
    def __init__(self):
        ...
        self.state_action_routing = nn.Sequential(
            Routing(d_cov=1, d_inp=4, d_out=32, n_out=4), #8
            Routing(d_cov=1, d_inp=32, d_out=32, n_inp=4, n_out=2),
        )
        self.value_final = nn.Linear(32, 1) # 1
    ...
    def forward(self, x):
        ...
        action_prob = F.softmax(state_caps, dim=-1).squeeze(0)
        state_caps = self.value_final(action_caps).squeeze(-1).squeeze(-1).squeeze(0)

    #F.softmax(action_caps, dim=-1)
...
def select_action(state):
    ...
    model.saved_actions.append(SavedAction(m.log_prob(action), state_value[action]))
