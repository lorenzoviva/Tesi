...
class DNC(nn.Module):
    ...
    def __init__(
        ...
        if output_size <= 0:
            self.final_output_size = self.input_size
        else:
        ...
        for layer in range(self.num_layers):
            ...
            if self.rnn_type.lower() == 'dnc':
                self.rnns.append(DNC((self.nn_input_size if layer == 0 else self.nn_output_size),
                                     self.output_size, bias=self.bias,  batch_first=True, dropout=self.dropout, num_layers=self.num_hidden_layers, gpu_id=self.gpu_id))
            if self.rnn_type.lower() == 'custom':
                self.rnns += self.controllers
        ...
        if chx is None:
            if self.rnn_type.lower() == 'custom':
                ...
                for rnn in self.rnns:
                    chx.append(rnn.init_hidden(batch_size))
            elif self.rnn_type.lower() == 'dnc':
                chx = []
                for rnn in self.rnns:
                    chx.append(rnn._init_hidden((None,None,None), batch_size, reset_experience))
            else:
                h = cuda(T.zeros(self.num_hidden_layers, batch_size, self.output_size), gpu_id=self.gpu_id)
                xavier_uniform_(h)
                if self.rnn_type.lower() == 'lstm':
                    chx = [(h, h)  for x in range(self.num_layers)]
                else:
                    chx = [h for x in range(self.num_layers)]
