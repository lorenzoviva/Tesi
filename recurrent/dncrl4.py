...
class DQNCartPoleSolver():
    def __init__(self, n_episodes=1000, n_win_ticks=195, max_env_steps=None, reccurrent_size=2, gamma=1.0, epsilon=1.0, epsilon_min=0.01, epsilon_log_decay=0.995, alpha=0.001, alpha_decay=0.01, batch_size=64, monitor=False, quiet=False):
        ...
        self.adapter.add(TimeDistributed(Flatten(input_shape=(None, 160,360,3)), input_shape=(self.reccurrent_size, 160,360,3), batch_size=batch_size))
        self.adapter.add(Dense(128, input_shape=(None, 172800), batch_size=batch_size))
        self.adapter.add(Dense(64, input_shape=(None, 128), batch_size=batch_size))
        self.adapter.add(Dense(32, input_shape=(None, 64), batch_size=batch_size))
        ...
        main_input = Input(shape=(self.reccurrent_size, 160, 360, 3), batch_size=batch_size)#(None, 32, ))
        dnc_cell = DNC(2, controller_units=30, **memory_config)
        ...
        # # adapted_input = self.adapter(main_input)
        # # ntm_cell = NTM(2, n_slots=50, m_depth=20, shift_range=3,
        # #                controller_model=None,
        # #                return_sequences=True,
        # #                input_shape=(None, 32),
        # #                batch_size = batch_size)
        # dnc_cell = DNC(2, controller_units=32)
        # dnc_initial_state = dnc_cell.get_initial_state(batch_size=batch_size)
        # self.rnn = Sequential()
        # self.rnn.add(dnc_cell)
        #
        # # self.model = dnc_cell(main_input,batch_size)
        # # layer = RNN(ntm_cell, batch_input_shape=(batch_size, 32))
        # # model = layer(adapted_input, prev_dnc_state=dnc_initial_state)
        # self.model = dnc_cell#Model(adapted_input, outputs=self.rnn)
        # # self.model = Model(main_input, dnc_cell)
        # # self.model._name = "gigino"
        # self.model.compile(optimizer=RMSprop(learning_rate=0.001),     loss='mse', run_eagerly=True)
